...
{
    public class PaymentRepository2 : IPaymentRepository2
    {
        private readonly ApplicationDbContext dbContext;
        private readonly AuthDbContext authDbContext;

        public PaymentRepository2(ApplicationDbContext dbContext, AuthDbContext authDbContext)
        {
        ...
        // Payment CRUD
        public async Task<Payment2?> GetPaymentByIdAsync(Guid id)
            => await dbContext.Payments2.FindAsync(id);

        public async Task<IEnumerable<Payment2>> GetAllPaymentsAsync()
            => await dbContext.Payments2.ToListAsync();

        public async Task<Payment2> AddPaymentAsync(Payment2 payment)
        {
            payment.Id = Guid.NewGuid();
            dbContext.Payments2.Add(payment);
            await dbContext.SaveChangesAsync();
            return payment;
        }

        public async Task<Payment2?> UpdatePaymentAsync(Guid id, Payment2 payment)
        {
            var existing = await dbContext.Payments2.FindAsync(id);
            if (existing == null) return null;
        ...
        public async Task<bool> DeletePaymentAsync(Guid id)
        {
            ...
            dbContext.Payments2.Remove(existing);
            await dbContext.SaveChangesAsync();
            return true;
        }
        ...
        public Task<Payment?> UpdatePaymentAsync(Guid id, Payment payment)
        {
            throw new NotImplementedException();
        }

        Task<Payment2?> IPaymentRepository2.GetPaymentByIdAsync(Guid id)
        {
            throw new NotImplementedException();
        }

        Task<IEnumerable<Payment2>> IPaymentRepository2.GetAllPaymentsAsync()
        {
            throw new NotImplementedException();
        }

        public Task<Payment2> AddPayment2Async(Payment2 payment)
        {
            throw new NotImplementedException();
        }
    }
